    ...
    <nav class="mb-6" aria-label="Breadcrumb">
        <ol class="flex items-center space-x-2 text-sm">
            ...
            <li class="flex items-center">
                ...
                <a href="{{ route('products.shop') ?? url('/shop') }}"
                    class="ml-2 text-gray-500 hover:text-blue-600 transition-colors">Shop</a>
            </li>
            @if ($product->category)
                <li class="flex items-center">
                    <svg class="w-4 h-4 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-4">
                    <div class="relative w-full h-96 bg-gray-50 rounded-lg overflow-hidden cursor-zoom-in"
                        ...
                        @php
                            ...
                            $mainImageUrl = $hasImages
                                ? (isset($images->first()->image_path)
                        ...
                        <div class="absolute top-4 left-4">
                            ...
                            @elseif($product->stock_quantity <= $product->reorder_level)
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-800 shadow-sm">
                                    Low Stock
                                </span>
                            @else
                                <span
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-4 mb-3">
                        <div class="flex items-center">
                            <span
                                class="text-2xl font-bold text-blue-600">৳{{ number_format($product->selling_price, 2) }}</span>
                            @if ($product->mrp && $product->mrp > $product->selling_price)
                                <span
                                    class="ml-2 text-lg text-gray-400 line-through">৳{{ number_format($product->mrp, 2) }}</span>
                    ...
                    <div class="space-y-3">
                        @if ($product->is_active && $product->stock_quantity > 0)
                            <button type="button" onclick="addToCart()"
                                class="w-full flex items-center justify-center px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold rounded-lg hover:from-blue-700 hover:to-blue-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
                            ...
                            <button type="button" onclick="buyNow()"
                                class="w-full flex items-center justify-center px-6 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white font-semibold rounded-lg hover:from-green-700 hover:to-green-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <ul class="mt-2 space-y-2">
                                    ...
                                    @if ($product->mrp)
                                        <li class="flex justify-between">
                                            ...
                                            <span
                                                class="font-medium line-through">৳{{ number_format($product->mrp, 2) }}</span>
                                        </li>
                                    @endif
                                    <li class="flex justify-between">
                ...
                <div id="shippingTab" class="tab-content hidden">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="text-gray-700">We offer a 7-day return policy for unused products in original
                                packaging. Contact our customer service for return requests.</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($relatedProducts as $relatedProduct)
                        <div
                            ...
                            <a href="{{ route('product.show', $relatedProduct->slug ?? $relatedProduct->id) }}"
                                class="block">
                                <div class="p-4">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <span
                                            class="text-lg font-bold text-blue-600">৳{{ number_format($relatedProduct->selling_price, 2) }}</span>
                                        @if ($relatedProduct->is_active && $relatedProduct->stock_quantity > 0)
                                            <button type="button" onclick="addRelatedToCart({{ $relatedProduct->id }})"
...
@push('scripts')
    <script>
        ...
        function showTab(tabName) {
            ...
            document.querySelector(`.tab-btn[data-tab="${tabName}"]`).classList.add('active', 'border-blue-600',
                'text-blue-600');
            document.querySelector(`.tab-btn[data-tab="${tabName}"]`).classList.remove('text-gray-500');
        }
                ...
                fetch('{{ route('product.notify') }}', {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            showNotification(data.message || 'Failed to save notification', 'error');
                        }
                    });
            } else if (email) {
        ...
        function showNotification(message, type = 'success') {
            ...
            notification.className =
                `fixed top-4 right-4 z-50 px-6 py-3 rounded-lg shadow-lg text-white font-medium transform transition-all duration-300 translate-x-full ${type === 'success' ? 'bg-green-500' : 'bg-red-500'}`;
