...
@section('content')
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Company & Branch</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                            ...
                            @forelse($users as $user)
                                <tr class="user-row hover:bg-gray-50 transition-colors duration-150"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @endphp
                                        <div class="relative inline-flex flex-col space-y-1 group">
    <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium {{ $config['color'] ?? 'bg-gray-100 text-gray-800' }}">
        ...
        <!-- Color indicators on the badge -->
        @foreach($user->roles->take(3) as $index => $role)
            @php
                $indicatorColors = ['bg-purple-500', 'bg-blue-500', 'bg-green-500', 'bg-yellow-500', 'bg-red-500', 'bg-indigo-500', 'bg-pink-500', 'bg-orange-500'];
                $indicatorColor = $indicatorColors[$index % count($indicatorColors)];
            @endphp
            <span class="ml-1 w-2 h-2 rounded-full {{ $indicatorColor }}"></span>
        @endforeach
        @if($user->roles->count() > 3)
            <span class="ml-1 text-[10px]">+{{ $user->roles->count() - 3 }}</span>
        @endif
    </span>
    ...
    <div class="absolute bottom-full left-0 mb-2 hidden group-hover:block z-10">
        <div class="bg-gray-900 text-white text-xs rounded-lg py-2 px-3 whitespace-nowrap">
            <div class="text-gray-400 text-[10px] mb-1">Assigned Roles:</div>
            @foreach($user->roles as $index => $role)
                @php
                    ...
                    $roleColor = $roleColors[$index % count($roleColors)];
                @endphp
                <div class="py-0.5 {{ $roleColor }}">• {{ $role->name }}</div>
            @endforeach
            <div class="absolute -bottom-1 left-3 w-2 h-2 bg-gray-900 transform rotate-45"></div>
        </div>
    </div>
</div>
                                    </td>
