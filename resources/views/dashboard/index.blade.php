    ...
    <div class="space-y-2">
            ...
            <div class="space-y-3">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 mt-1">Welcome back, {{ Auth::user()->name ?? 'User' }}! Here's what's happening today.</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            @foreach ($stats as $stat)
                <div class="group relative bg-white rounded-2xl p-6 border border-gray-100 hover:border-transparent transition-all duration-300 hover:shadow-xl overflow-hidden">
                        ...
                        <div class="flex items-start justify-between mb-6">
                            <div class="flex-1">
                                ...
                                <div class="flex items-center gap-2">
                                    <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold {{ $stat['trendColor'] === 'green' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700' }}">
                            ...
                            <div class="ml-4 p-3 rounded-xl bg-gradient-to-br from-{{ $stat['color'] }}-100 to-{{ $stat['color'] }}-50 group-hover:scale-110 transition-transform duration-300">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-2xl border border-gray-100 p-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                        ...
                        <div class="relative">
                            <select class="appearance-none bg-white border border-gray-200 rounded-lg px-4 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-100">
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-900">৳{{ number_format($averageRevenue ?? 0) }}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-900">৳{{ number_format($currentMonthRevenue ?? 0) }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500">Projected</p>
                                <p class="text-lg font-semibold text-blue-600">৳{{ number_format($projectedRevenue ?? 0) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        @php
                            $actions = [
                                ['route' => 'sales.sales-orders.create', 'icon' => 'plus', 'color' => 'blue', 'label' => 'New Order'],
                                ['route' => 'sales.invoices.create', 'icon' => 'file', 'color' => 'green', 'label' => 'New Invoice'],
                                ['route' => 'admin.companies.create', 'icon' => 'user-plus', 'color' => 'purple', 'label' => 'Add Customer'],
                                ['route' => 'inventory.products.create', 'icon' => 'package-plus', 'color' => 'amber', 'label' => 'Add Product'],
                            ];
                        @endphp
                        ...
                        @foreach ($actions as $action)
                            <a href="{{ route($action['route']) }}"
                               class="group flex flex-col items-center justify-center p-4 border border-gray-100 rounded-xl hover:border-{{ $action['color'] }}-200 hover:shadow-lg transition-all duration-200">
                                <div class="p-3 rounded-lg bg-{{ $action['color'] }}-50 group-hover:bg-{{ $action['color'] }}-100 transition-colors mb-3">
                                ...
                                <span class="text-sm font-medium text-gray-700 group-hover:text-{{ $action['color'] }}-700">{{ $action['label'] }}</span>
                            </a>
                        @endforeach
                    </div>
                </div>
                            ...
                            @foreach ($lowStockProducts->take(4) as $product)
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                    ...
                                    <div class="text-right">
                                        <span class="inline-block px-3 py-1 text-xs font-semibold rounded-full {{ $product->stock_quantity <= 0 ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800' }}">
                            ...
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                                <div class="flex items-center gap-4">
                                    <div class="flex-shrink-0">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center justify-center w-12 h-12 rounded-lg {{ $color }}">
                                            <span class="text-xs font-semibold">#{{ substr($invoice->invoice_number, -4) }}</span>
                                        </span>
                                    </div>
                                    <div>
                                        <a href="{{ route('sales.invoices.show', $invoice->id) }}"
                                           class="font-medium text-gray-900 hover:text-blue-600">{{ $invoice->invoice_number }}</a>
                                        <p class="text-sm text-gray-500">{{ $invoice->customer->customer_name ?? 'N/A' }}</p>
                                        <p class="text-xs text-gray-400">{{ $invoice->invoice_date->format('M d, Y') }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <div class="text-lg font-semibold text-gray-900">৳{{ number_format($invoice->total_amount, 2) }}</div>
                                    <div class="text-xs {{ $invoice->status === 'overdue' ? 'text-red-600' : 'text-gray-500' }}">
                            ...
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                                <div class="flex items-center gap-4">
                                    <div class="flex-shrink-0">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center justify-center w-12 h-12 rounded-lg {{ $color }}">
                                            <span class="text-xs font-semibold">#{{ substr($order->order_number, -4) }}</span>
                                        </span>
                                    </div>
                                    <div>
                                        <a href="{{ route('sales.sales-orders.show', $order->id) }}"
                                           class="font-medium text-gray-900 hover:text-blue-600">{{ $order->order_number }}</a>
                                        <p class="text-sm text-gray-500">{{ $order->customer->customer_name ?? 'N/A' }}</p>
